...
type GlobalServiceState = 'ready' | 'foreign' | 'unavailable';
type GlobalServiceLifecycleDependencies = {
  probe(): Promise<GlobalServiceState>;
  pathsExist(): Promise<boolean>;
...
export function createGlobalServiceLifecycle(dependencies: GlobalServiceLifecycleDependencies) {
  const now = dependencies.now || Date.now;
  const sleep = dependencies.sleep || Bun.sleep;
  const retryDelayMs = dependencies.retryDelayMs ?? 60_000;
  const pollAttempts = dependencies.pollAttempts ?? 20;
  const pollDelayMs = dependencies.pollDelayMs ?? 500;
  ...
  async function run(): Promise<boolean> {
    ...
    if (status === 'foreign' || now() < nextStartAt) return false;
    if (!(await dependencies.pathsExist())) return false;

    nextStartAt = now() + retryDelayMs;
    await dependencies.start();
    for (let attempt = 0; attempt < pollAttempts; attempt += 1) {
      await sleep(pollDelayMs);
      if ((await dependencies.probe()) === 'ready') return true;
    }
    return false;
  }
...
export async function extractGlobalMemory(
...
export async function postGlobalJob(
  job: string,
...
let sharedServiceLifecycle: ReturnType<typeof createGlobalServiceLifecycle> | undefined;
let sharedServiceLlm: LlmEnvironment | undefined;

function globalRuntimePaths() {
  const home = process.env.HOME || homedir();
  const dataHome = process.env.XDG_DATA_HOME || join(home, '.local', 'share');
  return {
    home,
    executable: join(dataHome, 'mac-setup', 'reme', 'venv', 'bin', 'reme'),
    config: join(home, '.config', 'reme', 'opencode-global.yaml'),
    workspace: join(dataHome, 'mac-setup', 'reme', 'global'),
    launcher: join(home, '.config', 'reme', 'start-global-service.sh'),
  };
}

async function probeGlobalService(expectedWorkspace: string): Promise<GlobalServiceState> {
  try {
    const status = globalServiceStatus(await postGlobalJob('app_config', {}, 1500), expectedWorkspace);
...
export function ensureGlobalService(llm: LlmEnvironment): Promise<boolean> {
  sharedServiceLlm = llm;
  if (!sharedServiceLifecycle) {
    const paths = globalRuntimePaths();
    sharedServiceLifecycle = createGlobalServiceLifecycle({
      probe: () => probeGlobalService(paths.workspace),
      pathsExist: async () =>
        (await Bun.file(paths.executable).exists()) &&
        (await Bun.file(paths.config).exists()) &&
        (await Bun.file(paths.launcher).exists()),
      start: () => {
        if (!sharedServiceLlm) throw new Error('ReMe service LLM environment is unavailable');
        const child = Bun.spawn(buildGlobalServiceArgs(paths.launcher, paths.executable, paths.config), {
          cwd: paths.home,
          detached: true,
          env: {
            ...process.env,
            LLM_BACKEND: sharedServiceLlm.backend,
            LLM_MODEL_NAME: sharedServiceLlm.model,
            LLM_API_KEY: sharedServiceLlm.apiKey,
            LLM_BASE_URL: sharedServiceLlm.baseURL,
          },
          stdin: 'ignore',
          stdout: 'ignore',
          stderr: 'ignore',
  ...
  return sharedServiceLifecycle.ensure();
}
...
export async function loadProjectMemoryNotes(workspace: string): Promise<ProjectMemoryNote[]> {
...
export async function retrieveMemoryForTurn(directory: string, query: string): Promise<string> {
  if (!query.trim() || containsSensitiveText(query)) return '';
  try {
    const workspace = await ensureProjectWorkspace(directory);
    const projectMemories = workspace
      ? rankProjectMemories(query, await loadProjectMemoryNotes(workspace))
      : [];
    let globalMemories: RetrievedMemory[] = [];
    try {
      const llm = resolveLlmEnvironment(process.env);
      if (llm && (await ensureGlobalService(llm))) {
        globalMemories = parseGlobalSearchResponse(
          await postGlobalJob('search', { query: query.slice(0, 4000), limit: 3 }, 1500),
        );
      }
    } catch {}
    return formatMemoryContext([...projectMemories, ...globalMemories]);
  } catch {
    return '';
  }
}

export async function captureMemoryTurn(request: {
  directory: string;
  sessionID: string;
  userText: string;
  ...
  const messages = buildMemoryMessages([
    {
      info: { id: 'user', role: 'user', time: { created: Date.now() } },
      parts: [{ type: 'text', text: request.userText }],
    },
    {
      info: { id: 'assistant', role: 'assistant', time: { created: Date.now() } },
      parts: [{ type: 'text', text: request.assistantText }],
    },
  ]);
  if (!messages || messages.length !== 2) return;

  const serviceReady = ensureGlobalService(llm);

  const home = process.env.HOME || homedir();
  const dataHome = process.env.XDG_DATA_HOME || join(home, '.local', 'share');
  const workspace = await ensureProjectWorkspace(request.directory).catch(() => null);
  if (workspace) {
      ...
      try {
        const child = Bun.spawn(
          buildProjectCaptureArgs(
            join(home, '.config', 'reme', 'start-global-service.sh'),
            join(dataHome, 'mac-setup', 'reme', 'venv', 'bin', 'python'),
            join(home, '.config', 'reme', 'run-project-capture.py'),
            join(home, '.config', 'reme', 'opencode-candidate.yaml'),
            workspace,
            request.sessionID,
          ),
          {
        ...
        child.stdin.end();
        await waitForSubprocess(child, DEFAULT_CAPTURE_TIMEOUT_MS);
      } catch {}
    }
  }

  try {
    const memory = await extractGlobalMemory(
      messages,
      llm,
      DEFAULT_EXTRACTION_TIMEOUT_MS,
      basename(request.directory),
    );
    if (!memory || !(await serviceReady)) return;
    await postGlobalJob(
      'auto_memory',
      {
        session_id: globalMemorySessionID(request.directory, request.sessionID),
