# Spec: OpenCode Active ReMe Memory

## Objective

Upgrade the OpenCode ReMe integration from project-only candidate capture to a two-layer active
memory system:
...
- Before each OpenCode model request, bounded project and global memory is injected as explicitly
  untrusted historical context.

The integration must remain best-effort. Memory, extraction, retrieval, service, or promotion
failures must never block the active OpenCode request.
...
```text
config/opencode/plugins/reme-memory.ts  Capture, service lifecycle, extraction, and retrieval
...
docs/reme-opencode-spec.md              Active-memory requirements and security boundaries
docs/reme-opencode-plan.md              Ordered implementation slices
...
1. OpenCode emits `session.idle`; the plugin debounces and cancels stale queued work when activity
   resumes.
...
1. Before each OpenCode model call, the plugin reads the latest ordinary user text as the query.
...
- Bun tests cover transcript filtering, global extraction parsing, provider-tier isolation,
  project lexical ranking, untrusted retrieval formatting, service arguments, and stale-work
  cancellation.
...
None. The user selected automatic global storage of cross-project experience only.
